var e=class{constructor(){this.allProducts=[],this.activeCategory=new URLSearchParams(window.location.search).get(`c`),this.init(),window.openQuickView=e=>this.openQuickView(e),window.closeQuickView=()=>this.closeQuickView()}async syncCollectionsGrid(){let e=document.getElementById(`categories-grid`);if(e)try{e.innerHTML=(await(await fetch(`/api/categories?_t=`+Date.now())).json()).map(e=>`
    ...
    `}async fetchAndRenderBlog(e){try{this.showLoading(e);let t=await(await fetch(`/api/blog?_t=`+Date.now())).json();if(!t||t.length===0){e.innerHTML=`<p class="no-products">Our editorial team is curating new stories. Check back soon!</p>`;return}e.innerHTML=`
        <div class="blog-grid">
          ${t.map(e=>`
            <article class="blog-card">
              <div class="blog-card-img">
                <img src="${e.coverImage||`/assets/editorial_v3/hero.png`}" alt="${e.title}" onerror="this.src='/assets/editorial_v3/hero.png'">
              </div>
              <div class="blog-card-content">
                <span class="blog-date">${new Date(e.createdAt).toLocaleDateString()}</span>
                <h3 class="blog-title">${e.title}</h3>
                <p class="blog-excerpt">${e.excerpt||``}</p>
                <a href="blog-post.html?slug=${e.slug}" class="blog-read-more">Read Full Story</a>
              </div>
            </article>
          `).join(``)}
        </div>
