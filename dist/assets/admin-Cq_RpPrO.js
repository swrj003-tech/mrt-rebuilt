import"./modulepreload-polyfill-EeOZK34R.js";var e=`/api`,t=localStorage.getItem(`mrt_admin_token`),n=`dashboard`;async function r(n,r={}){let i=new AbortController,a=setTimeout(()=>i.abort(),15e3),s={"Content-Type":`application/json`};t&&(s.Authorization=`Bearer ${t}`);let c=n,l=n.includes(`/products`)||n.includes(`/categories`)||n.includes(`/stats`)||n.includes(`/reviews`)||n.includes(`/messages`);(!r.method||r.method===`GET`)&&l&&(c+=(n.includes(`?`)?`&`:`?`)+`_t=${Date.now()}`);try{let t=await fetch(`${e}${c}`,{...r,headers:s,signal:i.signal});if(clearTimeout(a),t.status===401)return o(),null;if(t.status===204)return{success:!0};if(t.headers.get(`content-length`)===`0`)return t.ok?{success:!0}:{error:`Request failed`};let n=await t.text();if(!n||n.trim()===``)return t.ok?{success:!0}:{error:`Empty response`};let l;try{l=JSON.parse(n)}catch{return t.ok?{success:!0}:{error:n}}return t.ok||console.error(`API Error [${t.status}]:`,l),l}catch(e){return clearTimeout(a),e.name===`AbortError`?(console.error(`API Timeout [${n}]`),{error:`Request timed out. Please check your connection or server status.`}):(console.error(`API Fetch Error [${n}]:`,e),{error:`Network or parse error`})}}function i(e,t=`success`){document.querySelectorAll(`.toast`).forEach(e=>e.remove());let n=document.createElement(`div`);n.className=`toast ${t}`,n.innerHTML=`<span class="material-symbols-outlined">${t===`success`?`check_circle`:`error`}</span> ${e}`,document.body.appendChild(n),setTimeout(()=>n.remove(),3200)}function a(e){n=e,s()}function o(){t=null,localStorage.removeItem(`mrt_admin_token`),s()}function s(){let e=document.getElementById(`app`);if(!t){c(e);return}l(e)}function c(e){e.innerHTML=`
  ...
  `,u(),d();let t=document.getElementById(`mobile-nav-toggle`),n=document.getElementById(`admin-overlay`),r=document.getElementById(`admin-sidebar`);t&&t.addEventListener(`click`,()=>{r.classList.toggle(`show`),n.classList.toggle(`show`)}),n&&n.addEventListener(`click`,()=>{r.classList.remove(`show`),n.classList.remove(`show`)})}function u(){let e=document.getElementById(`admin-sidebar`);e&&(e.innerHTML=`
    ...
    <div class="sidebar-section">
      ...
      <a class="nav-item ${n===`blog`?`active`:``}" data-view="blog">
  ...
  `,e.querySelectorAll(`.nav-item[data-view]`).forEach(t=>{t.addEventListener(`click`,()=>{let n=t.dataset.view;if(n===`logout`){o();return}a(n),e.classList.remove(`show`);let r=document.getElementById(`admin-overlay`);r&&r.classList.remove(`show`)})}))}async function d(){let e=document.getElementById(`main-content`);if(!e)return;let t=document.getElementById(`view-${n}`);switch(t||(t=document.createElement(`div`),t.id=`view-${n}`,t.className=`view-container`,e.appendChild(t)),document.querySelectorAll(`.view-container`).forEach(e=>e.style.display=`none`),t.style.display=`block`,t.innerHTML=`<div class="text-center mt-2"><div class="loading-spinner"></div></div>`,n){case`dashboard`:await f(t);break;case`blog`:await _(t);break;case`products`:await p(t);break;case`categories`:await h(t);break;case`testimonials`:await y(t);break;case`analytics`:await w(t);break;case`reviews`:await T(t);break;case`messages`:await C(t);break;case`newsletter`:await x(t);break;default:await f(t)}}async function f(e){let t=await r(`/admin/stats`);t&&(e.innerHTML=`
            ...
            <tr>
              <td>
                <div class="flex gap-1" style="align-items:center">
                  <img src="${e.productImage||e.product?.image||`https://placehold.co/150x150?text=No+Image`}" class="product-thumb" alt="" onerror="this.src='https://placehold.co/150x150?text=No+Image'">
                  <span>${e.productName||e.product?.name||`Unknown`}</span>
                </div>
              </td>
              <td><span class="badge badge-info">Affiliate Click</span></td>
              <td class="text-muted text-sm">${new Date(e.clickedAt||e.createdAt).toLocaleString()}</td>
            </tr>
          `).join(``)||`<tr><td colspan="3" class="text-center text-muted" style="padding:2rem">No recent activity</td></tr>`}
        </tbody>
      </table>
    </div>
<<<<<<<< HEAD:dist/assets/admin-CNW0veMw.js
  `)}async function p(e){let t=await r(`/products`);if(await r(`/categories`),!t)return;let n=t.products||[];window._allProducts=n,e.innerHTML=`
========
  `)}async function p(e){let t=await r(`/products?limit=1000`);if(await r(`/categories`),!t)return;let n=t.products||[];window._allProducts=n,e.innerHTML=`
>>>>>>>> c7256ed (pushing changes made by codex - including CMS fixes, product display updates, and data sync scripts):dist/assets/admin-Cq_RpPrO.js
    <div class="page-header">
    ...
    <div class="table-container">
      <table>
        ...
        <tbody>
          ${n.map(e=>`
            <tr>
              <td><img src="${e.image||`https://placehold.co/150x150?text=No+Image`}" class="product-thumb" alt="${e.name}" onerror="this.src='https://placehold.co/150x150?text=No+Image'"></td>
              <td>
  ...
  `}function m(e,t){let n=!!e,a=document.createElement(`div`);a.className=`modal-overlay`,a.innerHTML=`
    <div class="modal">
      ...
      <form id="product-form">
        ...
        <div class="grid-2">
          <div class="form-group">
            ...
            <select name="categoryId" required>
              ${(Array.isArray(t)?t:[]).map(t=>`<option value="${t.id}" ${e?.categoryId===t.id?`selected`:``}>${t.name}</option>`).join(``)}
            </select>
          </div>
          <div class="form-group">
            <label>Badge</label>
        ...
        <div class="form-group">
          ...
          <textarea name="keyBenefits" placeholder="Fast charging&#10;Compact design&#10;Universal compatibility">${(()=>{let t=e?.keyBenefits||[];if(typeof t==`string`&&t.startsWith(`[`))try{t=JSON.parse(t)}catch{t=[t]}return Array.isArray(t)?t.join(`
`):t})()}</textarea>
        </div>
...
`).filter(Boolean),c.price=c.price?parseFloat(c.price):null,c.originalPrice=c.originalPrice?parseFloat(c.originalPrice):null,c.ratingValue=parseFloat(c.ratingValue)||0,c.currency=c.currency||`USD`;let l;if(n){if(l=await r(`/products/${e.id}`,{method:`PUT`,body:JSON.stringify(c)}),l?.error){i(l.error,`error`),o.disabled=!1,o.innerHTML=`<span class="material-symbols-outlined">save</span> Update Product`;return}i(`Product updated`)}else{if(l=await r(`/products`,{method:`POST`,body:JSON.stringify(c)}),l?.error){i(l.error,`error`),o.disabled=!1,o.innerHTML=`<span class="material-symbols-outlined">add_circle</span> Create Product`;return}i(`Product created`)}a.remove(),setTimeout(()=>d(),100)})}async function h(e){let t=await r(`/categories`);t&&(e.innerHTML=`
    ...
    <div class="admin-blog-list">
      ${t.map(e=>`
        <div class="admin-blog-card">
          <div class="admin-blog-img">
            <img src="${e.coverImage||``}" alt="" onerror="this.style.opacity='0.2'">
          </div>
          <div class="admin-blog-content">
            <div class="admin-blog-main">
              <div class="admin-blog-info">
                <h3 class="admin-blog-title">${e.title}</h3>
                <div class="admin-blog-meta">
                  <span class="badge ${e.isPublished?`badge-success`:`badge-warning`}">${e.isPublished?`Published`:`Draft`}</span>
                  <span class="text-muted text-sm">${new Date(e.createdAt).toLocaleDateString()}</span>
                  <span class="text-muted text-sm">· By ${e.author}</span>
                </div>
                <p class="admin-blog-excerpt">${e.content?e.content.replace(/<[^>]*>?/gm,``):e.excerpt||`No content provided.`}</p>
              </div>
              <div class="action-btns">
                <button class="btn btn-sm btn-secondary btn-edit-blog" data-id="${e.id}" title="Edit"><span class="material-symbols-outlined">edit</span> <span class="btn-label">Edit</span></button>
                <button class="btn btn-sm btn-danger btn-delete-blog" data-id="${e.id}" title="Delete"><span class="material-symbols-outlined">delete</span></button>
              </div>
            </div>
          </div>
        </div>
      `).join(``)||`<div class="text-center text-muted" style="padding:4rem">No posts yet</div>`}
    </div>
  `)}function v(e){let t=!!e,n=document.createElement(`div`);n.className=`modal-overlay`,n.innerHTML=`
    <div class="modal" style="max-width:800px; width:95%;">
      <div class="modal-header">
        <h3>${t?`Edit Blog Post`:`Add Blog Post`}</h3>
        <button class="modal-close-btn" id="modal-close-x" type="button"><span class="material-symbols-outlined">close</span></button>
      </div>
      <form id="blog-form">
        <div class="grid-2">
          <div class="form-group"><label>Title</label><input type="text" name="title" value="${e?.title||``}" required></div>
          <div class="form-group"><label>Slug</label><input type="text" name="slug" value="${e?.slug||``}" placeholder="auto-generated"></div>
        </div>
        <div class="form-group">
          <label>Cover Image URL</label>
          <input type="text" name="coverImage" value="${e?.coverImage||``}" placeholder="https://example.com/image.jpg">
        </div>
        <div class="grid-2">
          <div class="form-group"><label>Author</label><input type="text" name="author" value="${e?.author||`MRT Editorial`}"></div>
          <div class="form-group" style="display:flex;align-items:center;gap:0.5rem;padding-top:2rem">
            <input type="checkbox" id="isPublished" name="isPublished" ${e?.isPublished?`checked`:``} style="width:auto;height:auto;transform:scale(1.2)">
            <label for="isPublished" style="margin:0;font-weight:700;color:var(--ink)">Published & Visible on Site</label>
          </div>
        </div>
        <div class="form-group"><label>Excerpt</label><textarea name="excerpt" style="height:60px">${e?.excerpt||``}</textarea></div>
        <div class="form-group"><label>HTML Content</label><textarea name="content" required style="height:350px;font-family:monospace;font-size:13px">${e?.content||``}</textarea></div>
        
        <div class="modal-actions">
          <button type="button" class="btn btn-secondary" id="modal-cancel">Cancel</button>
          <button type="submit" class="btn btn-primary">${t?`Update`:`Create`} Post</button>
        </div>
      </form>
    </div>
  `,document.body.appendChild(n),n.querySelector(`#modal-cancel`).addEventListener(`click`,()=>n.remove()),n.querySelector(`#modal-close-x`).addEventListener(`click`,()=>n.remove()),n.addEventListener(`click`,e=>{e.target===n&&n.remove()}),n.querySelector(`#blog-form`).addEventListener(`submit`,async a=>{a.preventDefault();let o=new FormData(a.target),s=Object.fromEntries(o.entries());s.isPublished=a.target.querySelector(`#isPublished`).checked;let c=a.target.querySelector(`button[type="submit"]`);c.disabled=!0,c.innerHTML=`<div class="loading-spinner" style="width:16px;height:16px"></div> Saving...`;let l;if(l=t?await r(`/blog/${e.id}`,{method:`PUT`,body:JSON.stringify(s)}):await r(`/blog`,{method:`POST`,body:JSON.stringify(s)}),l?.error){i(l.error,`error`),c.disabled=!1,c.innerHTML=t?`Update Post`:`Create Post`;return}i(`Post ${t?`updated`:`created`}`),n.remove(),d()})}async function y(e){let t=await r(`/testimonials`);t&&(e.innerHTML=`
    <div class="page-header">
  ...
  `)}function b(){let e=document.createElement(`div`);e.className=`modal-overlay`,e.innerHTML=`
    <div class="modal">
      <div class="modal-header">
  ...
  `,document.querySelectorAll(`.btn-del-sub`).forEach(e=>e.addEventListener(`click`,async()=>{if(confirm(`Remove subscriber?`)){if((await r(`/newsletter/subscribers/${e.dataset.id}`,{method:`DELETE`}))?.error){i(`Remove failed`,`error`);return}i(`Subscriber removed`),d()}}))}async function S(){let e=await r(`/admin/messages`);return Array.isArray(e)?e:[]}async function C(e){let t=await S();e.innerHTML=`
    <div class="page-header">
  ...
  `}async function w(e){let t=await r(`/admin/analytics`),n=await r(`/admin/stats`);!t||!n||(e.innerHTML=`
    <div class="page-header">
        ...
        <tbody>
          ${(t.clicks||[]).slice(0,10).map(e=>`
            <tr>
              ...
              <td class="text-xs">${new Date(e.clickedAt||e.createdAt).toLocaleString()}</td>
            </tr>
          `).join(``)||`<tr><td colspan="4" class="text-center text-muted" style="padding:2rem">No clicks recorded</td></tr>`}
        </tbody>
      </table>
    </div>
  `)}async function T(e){let t=await r(`/admin/reviews`);t&&(e.innerHTML=`
    <div class="page-header">
            ...
            <tr>
              ...
              <td>
                <div class="action-btns">
                  ...
                  <button class="btn btn-sm btn-secondary btn-edit-review" data-id="${e.id}" data-username="${e.userName}" data-rating="${e.rating}" data-comment="${encodeURIComponent(e.comment)}" title="Edit"><span class="material-symbols-outlined">edit</span></button>
...
<<<<<<<< HEAD:dist/assets/admin-CNW0veMw.js
  `,document.querySelectorAll(`.btn-verify-review`).forEach(e=>e.addEventListener(`click`,async()=>{await r(`/admin/reviews/${e.dataset.id}/verify`,{method:`POST`}),i(`Review verified`),d()})),document.querySelectorAll(`.btn-delete-review`).forEach(e=>e.addEventListener(`click`,async()=>{if(confirm(`Delete this review?`)){if((await r(`/reviews/${e.dataset.id}`,{method:`DELETE`}))?.error){i(`Delete failed`,`error`);return}i(`Review deleted`),d()}})))}function T(n,r){[`dragenter`,`dragover`,`dragleave`,`drop`].forEach(e=>{n.addEventListener(e,e=>{e.preventDefault(),e.stopPropagation()})}),n.addEventListener(`dragover`,()=>n.classList.add(`drag-active`)),n.addEventListener(`dragleave`,()=>n.classList.remove(`drag-active`)),n.addEventListener(`drop`,async a=>{n.classList.remove(`drag-active`);let o=Array.from(a.dataTransfer.files);if(!o.length)return;let s=new FormData;o.forEach(e=>s.append(`files`,e)),n.innerHTML=`<div class="loading-spinner"></div>`;try{let n=await(await fetch(`${e}/media/upload`,{method:`POST`,headers:{Authorization:`Bearer ${t}`},body:s})).json();n&&n[0]?r(n[0].url):i(`Upload failed`,`error`)}catch{i(`Upload error`,`error`)}finally{n.innerHTML=`<span class="material-symbols-outlined">upload_file</span><span>Upload Completed ✓</span>`}}),n.addEventListener(`click`,()=>{let a=document.createElement(`input`);a.type=`file`,a.accept=`image/*`,a.onchange=async()=>{let o=a.files[0];if(!o)return;let s=new FormData;s.append(`files`,o),n.innerHTML=`<div class="loading-spinner"></div>`;try{let n=await(await fetch(`${e}/media/upload`,{method:`POST`,headers:{Authorization:`Bearer ${t}`},body:s})).json();n&&n[0]&&r(n[0].url)}catch{i(`Upload error`,`error`)}finally{n.innerHTML=`<span class="material-symbols-outlined">upload_file</span><span>Upload Completed ✓</span>`}},a.click()})}function E(e){return new Promise(t=>{let n=document.createElement(`div`);n.className=`modal-overlay`,n.style.zIndex=`2000`,n.innerHTML=`
========
  `,document.querySelectorAll(`.btn-edit-review`).forEach(e=>e.addEventListener(`click`,()=>{D({id:e.dataset.id,userName:e.dataset.username,rating:Number(e.dataset.rating),comment:decodeURIComponent(e.dataset.comment)})})))}function E(n,r){[`dragenter`,`dragover`,`dragleave`,`drop`].forEach(e=>{n.addEventListener(e,e=>{e.preventDefault(),e.stopPropagation()})}),n.addEventListener(`dragover`,()=>n.classList.add(`drag-active`)),n.addEventListener(`dragleave`,()=>n.classList.remove(`drag-active`)),n.addEventListener(`drop`,async a=>{n.classList.remove(`drag-active`);let o=Array.from(a.dataTransfer.files);if(!o.length)return;let s=new FormData;o.forEach(e=>s.append(`files`,e)),n.innerHTML=`<div class="loading-spinner"></div>`;try{let n=await(await fetch(`${e}/media/upload`,{method:`POST`,headers:{Authorization:`Bearer ${t}`},body:s})).json();n&&n[0]?r(n[0].url):i(`Upload failed`,`error`)}catch{i(`Upload error`,`error`)}finally{n.innerHTML=`<span class="material-symbols-outlined">upload_file</span><span>Upload Completed ✓</span>`}}),n.addEventListener(`click`,()=>{let a=document.createElement(`input`);a.type=`file`,a.accept=`image/*`,a.onchange=async()=>{let o=a.files[0];if(!o)return;let s=new FormData;s.append(`files`,o),n.innerHTML=`<div class="loading-spinner"></div>`;try{let n=await(await fetch(`${e}/media/upload`,{method:`POST`,headers:{Authorization:`Bearer ${t}`},body:s})).json();n&&n[0]&&r(n[0].url)}catch{i(`Upload error`,`error`)}finally{n.innerHTML=`<span class="material-symbols-outlined">upload_file</span><span>Upload Completed ✓</span>`}},a.click()})}function D(e){let t=document.createElement(`div`);t.className=`modal-overlay`,t.innerHTML=`
    <div class="modal" style="max-width:500px;">
      ...
      <div class="modal-body" style="display:flex;flex-direction:column;gap:1rem;">
        <div class="form-group">
          ...
          <input type="text" class="form-input" id="edit-review-username" value="${e.userName}" maxlength="120">
        </div>
        <div class="form-group">
          <label class="form-label">Rating (1–5)</label>
          <input type="number" class="form-input" id="edit-review-rating" value="${e.rating}" min="1" max="5" step="1">
        </div>
        <div class="form-group">
          <label class="form-label">Comment</label>
          <textarea class="form-input" id="edit-review-comment" rows="4" maxlength="2000">${e.comment}</textarea>
        </div>
      </div>
      <div class="modal-actions">
  ...
  `,document.body.appendChild(t),t.querySelector(`#close-review-modal`).addEventListener(`click`,()=>t.remove()),t.querySelector(`#cancel-review-edit`).addEventListener(`click`,()=>t.remove()),t.addEventListener(`click`,e=>{e.target===t&&t.remove()}),t.querySelector(`#save-review-edit`).addEventListener(`click`,async()=>{let n=t.querySelector(`#edit-review-username`).value.trim(),a=Number(t.querySelector(`#edit-review-rating`).value),o=t.querySelector(`#edit-review-comment`).value.trim();if(!n||!o||a<1||a>5){i(`Please fill all fields with valid values`,`error`);return}let s=t.querySelector(`#save-review-edit`);s.disabled=!0,s.textContent=`Saving…`;let c=await r(`/admin/reviews/${e.id}`,{method:`PUT`,body:JSON.stringify({userName:n,rating:a,comment:o})});if(c?.error){i(c.error,`error`),s.disabled=!1,s.textContent=`Save Changes`;return}i(`Review updated`),t.remove(),d()})}function O(e){return new Promise(t=>{let n=document.createElement(`div`);n.className=`modal-overlay`,n.style.zIndex=`2000`,n.innerHTML=`
>>>>>>>> c7256ed (pushing changes made by codex - including CMS fixes, product display updates, and data sync scripts):dist/assets/admin-Cq_RpPrO.js
      <div class="modal" style="max-width: 400px; text-align: center; padding: 2rem;">
        ...
        <p style="margin-bottom: 2rem; color: var(--text-secondary); line-height: 1.6;">${e}</p>
...
<<<<<<<< HEAD:dist/assets/admin-CNW0veMw.js
    `,document.body.appendChild(n);let r=e=>{n.remove(),t(e)};n.querySelector(`#confirm-cancel`).addEventListener(`click`,()=>r(!1)),n.querySelector(`#confirm-yes`).addEventListener(`click`,()=>r(!0)),n.addEventListener(`click`,e=>{e.target===n&&r(!1)})})}document.addEventListener(`click`,async e=>{let t=e.target,n=t.closest(`.btn-edit-product`);if(n){let e=(window._allProducts||[]).find(e=>String(e.id)===String(n.dataset.id));e&&m(e,await r(`/categories`));return}let a=t.closest(`.btn-delete-product`);if(a){if(!await E(`Are you sure you want to delete this product?`))return;a.disabled=!0;let e=await r(`/products/${a.dataset.id}`,{method:`DELETE`});if(e?.error){i(e.error,`error`),a.disabled=!1;return}i(`Product deleted`),d();return}if(t.closest(`#btn-add-product`)){m(null,await r(`/categories`));return}let o=t.closest(`.btn-edit-cat`);if(o){let e=(await r(`/categories`)||[]).find(e=>String(e.id)===String(o.dataset.id));e&&g(e);return}let s=t.closest(`.btn-delete-cat`);if(s){if(!await E(`Delete this category? This will also remove all associated products.`))return;s.disabled=!0;let e=await r(`/categories/${s.dataset.id}`,{method:`DELETE`});if(e?.error){i(e.error,`error`),s.disabled=!1;return}i(`Category deleted`),d();return}if(t.closest(`#btn-add-cat`)){g(null);return}let c=t.closest(`.btn-verify-review`);if(c){let e=await r(`/admin/reviews/${c.dataset.id}/verify`,{method:`POST`});if(e?.error)return i(e.error,`error`);i(`Review verified`),d();return}let l=t.closest(`.btn-delete-review`);if(l){if(!await E(`Delete this review?`))return;let e=await r(`/reviews/${l.dataset.id}`,{method:`DELETE`});if(e?.error)return i(e.error,`error`);i(`Review deleted`),d();return}}),s();
========
    `,document.body.appendChild(n);let r=e=>{n.remove(),t(e)};n.querySelector(`#confirm-cancel`).addEventListener(`click`,()=>r(!1)),n.querySelector(`#confirm-yes`).addEventListener(`click`,()=>r(!0)),n.addEventListener(`click`,e=>{e.target===n&&r(!1)})})}document.addEventListener(`click`,async e=>{let t=e.target;try{let e=t.closest(`.btn-edit-product`);if(e){let t=e.dataset.id,n=(window._allProducts||[]).find(e=>String(e.id)===String(t));if(n){let e=await r(`/categories`);e&&!e.error?m(n,e):i(`Failed to load categories`,`error`)}else console.warn(`Product not found in window._allProducts:`,t);return}let n=t.closest(`.btn-delete-product`);if(n){if(!await O(`Are you sure you want to delete this product?`))return;n.disabled=!0;let e=await r(`/products/${n.dataset.id}`,{method:`DELETE`});if(e?.error){i(e.error,`error`),n.disabled=!1;return}i(`Product deleted`),d();return}if(t.closest(`#btn-add-product`)){let e=await r(`/categories`);e&&!e.error?m(null,e):i(`Failed to load categories`,`error`);return}let a=t.closest(`.btn-edit-cat`);if(a){let e=await r(`/categories`),t=(Array.isArray(e)?e:[]).find(e=>String(e.id)===String(a.dataset.id));t&&g(t);return}let o=t.closest(`.btn-delete-cat`);if(o){if(!await O(`Delete this category? This will also remove all associated products.`))return;o.disabled=!0;let e=await r(`/categories/${o.dataset.id}`,{method:`DELETE`});if(e?.error){i(e.error,`error`),o.disabled=!1;return}i(`Category deleted`),d();return}if(t.closest(`#btn-add-cat`)){g(null);return}let s=t.closest(`.btn-edit-blog`);if(s){let e=(await r(`/blog/admin/all`)||[]).find(e=>String(e.id)===String(s.dataset.id));e&&v(e);return}let c=t.closest(`.btn-delete-blog`);if(c){if(!await O(`Are you sure you want to delete this blog post?`))return;c.disabled=!0;let e=await r(`/blog/${c.dataset.id}`,{method:`DELETE`});if(e?.error){i(e.error,`error`),c.disabled=!1;return}i(`Post deleted`),d();return}if(t.closest(`#btn-add-blog`)){v(null);return}let l=t.closest(`.btn-del-test`);if(l){if(!await O(`Delete this testimonial?`))return;l.disabled=!0;let e=await r(`/testimonials/${l.dataset.id}`,{method:`DELETE`});if(e?.error){i(e.error,`error`),l.disabled=!1;return}i(`Testimonial deleted`),d();return}if(t.closest(`#btn-add-test`)){b();return}let u=t.closest(`.btn-verify-review`);if(u){let e=await r(`/admin/reviews/${u.dataset.id}/verify`,{method:`POST`});if(e?.error)return i(e.error,`error`);i(`Review verified`),d();return}let f=t.closest(`.btn-delete-review`);if(f){if(!await O(`Delete this review?`))return;let e=await r(`/reviews/${f.dataset.id}`,{method:`DELETE`});if(e?.error)return i(e.error,`error`);i(`Review deleted`),d();return}}catch(e){console.error(`Global Click Delegator Error:`,e),i(`An unexpected error occurred`,`error`)}}),s();
>>>>>>>> c7256ed (pushing changes made by codex - including CMS fixes, product display updates, and data sync scripts):dist/assets/admin-Cq_RpPrO.js
