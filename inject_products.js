...
async function seed() {
  console.log('🚀 Starting Mass Product Injection...');
  
  const [tables] = await pool.query('SHOW TABLES');
  const rawList = tables.map(t => Object.values(t)[0]);
  const lowerList = rawList.map(t => t.toLowerCase());
  const productTable = rawList[lowerList.indexOf('product')];
  const categoryTable = rawList[lowerList.indexOf('category')];
  ...
  console.log(`🧹 Cleaning existing products in ${productTable}...`);
  await pool.query(`DELETE FROM ${productTable}`);

  for (const p of products) {
    const catId = catMap[p.category.toLowerCase()];
    if (!catId) {
      console.warn(`⚠️ Skipping ${p.name}: Category ${p.category} not found.`);
      continue;
    }
