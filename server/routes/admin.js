...
router.get('/', (req, res) => res.json({ status: 'MRT Admin API Active', version: '3.0.0' }));

// GET /api/admin/force-sync-products - FULL 105 CATALOG WITH AI-MATCHED IMAGES
router.get('/force-sync-products', async (req, res) => {
  try {
    const t = await getTables();
    
    // AI-MATCHED IMAGE DATABASE (MAPPED TO YOUR 105 PRODUCTS)
    const products = [
      // HOME & KITCHEN (15)
    ...
    // SAFE Category Visual Preservation
    /* Category images are NOT overwritten here to preserve user's custom designs */
    ...
    res.json({ success: true, message: `Matched 105 products with premium visuals and injected successfully.` });
  } catch (err) {
